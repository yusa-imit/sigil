//! Tiger Style tidy lint for sigil: line length, doc headers, function length
//! (with a shrink-only baseline), a substring ban list, file length, and a
//! sigil-specific check banning `usize` in wire/serialized format types.
...
//! matching the vendored reference's own convention.
...
/// sigil's format modules (`REALM.md`: "Format modules are siblings") — every
/// path rooted here is a serialized wire/file format type and may never
/// declare a `usize` field, since its width varies across build targets.
const wire_format_names = [_][]const u8{
    "json", "toml", "yaml", "msgpack", "cbor", "proto", "csv",
};

const wire_format_path_buf_len: usize = 32;

comptime {
    // Guarantees bufPrint below can never truncate: a future longer format
    // name must grow this buffer, not silently stop matching (no silent
    // truncation, per Tiger Style).
    for (wire_format_names) |name| {
        std.debug.assert(name.len + "src/.zig".len <= wire_format_path_buf_len);
    }
}
...
/// True when `line` (already known to be under a wire-format path) looks
/// like a struct field declaration (`name: Type,` or `name: Type = x,`)
/// rather than a local variable, function parameter, or return type — the
/// `wire_usize` rule only makes sense for serialized field types. A known
/// line-oriented limitation: a multi-line parameter list's continuation
/// line (`i: usize,`) looks identical to a field and is still flagged.
fn looksLikeStructField(line: []const u8) bool {
    const trimmed = std.mem.trim(u8, line, " \t");
    if (!std.mem.endsWith(u8, trimmed, ",")) return false;
    if (std.mem.indexOf(u8, trimmed, "fn ") != null) return false;
    if (std.mem.startsWith(u8, trimmed, "var ") or std.mem.startsWith(u8, trimmed, "const "))
        return false;
    return true;
}
...
/// Runs all checks over one file's already-split `lines`, appending every
/// finding into `findings` and recording function spans into `actual_out`
/// for the caller's later `reconcileBaseline` pass.
pub fn lintLines(
    allocator: Allocator,
    path: []const u8,
    lines: []const []const u8,
...
test "checkBanList flags `: usize` only inside a wire-format module" {
    const gpa = std.testing.allocator;
    const field_line = "    count: usize,";

    const in_json = try checkBanList(gpa, "src/json.zig", &.{field_line});
    defer freeFindings(gpa, in_json);
    try std.testing.expectEqual(@as(usize, 1), in_json.len);

    const in_json_submodule = try checkBanList(gpa, "src/json/dom.zig", &.{field_line});
    defer freeFindings(gpa, in_json_submodule);
    try std.testing.expectEqual(@as(usize, 1), in_json_submodule.len);

    const in_core = try checkBanList(gpa, "src/core.zig", &.{field_line});
    defer freeFindings(gpa, in_core);
    try std.testing.expectEqual(@as(usize, 0), in_core.len);

    const in_main = try checkBanList(gpa, "src/main.zig", &.{field_line});
    defer freeFindings(gpa, in_main);
    try std.testing.expectEqual(@as(usize, 0), in_main.len);
}

test "checkBanList allows `usize` locals and params in wire-format modules" {
    const gpa = std.testing.allocator;

    const local_var = try checkBanList(gpa, "src/json.zig", &.{"    var pos: usize = 0;"});
    defer freeFindings(gpa, local_var);
    try std.testing.expectEqual(@as(usize, 0), local_var.len);

    const param = try checkBanList(
        gpa,
        "src/json.zig",
        &.{"fn scan(self: *Scanner, i: usize) !Token {"},
    );
    defer freeFindings(gpa, param);
    try std.testing.expectEqual(@as(usize, 0), param.len);
}

test "isWireFormatPath does not match near-miss filenames" {
    try std.testing.expect(!isWireFormatPath("src/jsonlines.zig"));
    try std.testing.expect(!isWireFormatPath("src/csv2.zig"));
    try std.testing.expect(!isWireFormatPath("src/config.zig"));
    try std.testing.expect(isWireFormatPath("src/csv.zig"));
    try std.testing.expect(isWireFormatPath("src/csv/writer.zig"));
}
